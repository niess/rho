...
float distance_next_voxel(float x, float y, float z, // position
        ...
        float t[3];
        t[0] = t[1] = t[2] = INFINITY;

        const float u[3] = { ux, uy, uz }, r0[3] = { x0, y0, z0 };
        const float r[3] = { x, y, z }, size[3] = { sizex, sizey, sizez };

        // check planes in Ox, Oy and Oz directions
        for (int i = 0; i < 3; i++) {
                if(u[i] != 0) {
                        t[i] = (r0[i] + 0.5 * size[i] - r[i]) / u[i];
                        if(t[i] < 0) {
                                t[i] = (r0[i] - 0.5 * size[i] - r[i]) / u[i];
                        }
                }

                if ((t[i] > 0) && (t[i] < distance))
                        distance = t[i];
        }
